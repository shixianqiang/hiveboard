/*
 * linux/arch/unicore/kernel/asm-offsets.c
 *
...
int main(void)
{
  ...
  DEFINE(S_PSR,			offsetof(struct pt_regs, UCreg_csr));
  ...
  DEFINE(SWSUSP_CPU,		offsetof(struct swsusp_arch_regs, cpu_context));
#ifdef	CONFIG_UNICORE_FPU_F64
  DEFINE(SWSUSP_FPSTATE,	offsetof(struct swsusp_arch_regs, fpstate));
#endif
  return 0; 
}
