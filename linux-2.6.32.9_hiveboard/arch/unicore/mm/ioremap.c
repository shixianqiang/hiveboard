/*
 * linux/arch/unicore/mm/ioremap.c
 *
			  ...
			  unsigned long phys_addr, const struct mem_type *type)
{
	...
	do {
		...
		set_pte(pte, pfn_pte(phys_addr >> PAGE_SHIFT, prot));
		phys_addr += PAGE_SIZE;
				 ...
				 const struct mem_type *type)
{
	...
	pmd = pmd_alloc(&init_mm, (pud_t *)pgd, addr);
	if (!pmd)
		return -ENOMEM;
 ...
 */
void __iomem *
__unicore_ioremap_pfn(unsigned long pfn, unsigned long offset, size_t size,
		  unsigned int mtype)
{
	const struct mem_type *type;
	int err;
...
EXPORT_SYMBOL(__unicore_ioremap_pfn);

void __iomem *
__unicore_ioremap(unsigned long phys_addr, size_t size, unsigned int mtype)
{
    /*
     * Added by wanghuan, the io address doesn't need page
     * the virtural address = physical address & 0x7FFFFFFF
     */
  //  return phys_addr & 0x7FFFFFFF;
 /* Added by wanghuan */
	...
	if (!size || last_addr < phys_addr)
 	...
 	return __unicore_ioremap_pfn(pfn, offset, size, mtype);

}
EXPORT_SYMBOL(__unicore_ioremap);
