...
static irqreturn_t usb3343_irq(int irq, void *handle);
...
void musb_platform_enable(struct musb *musb)
{
	...
	printk(KERN_WARNING "中断寄存器 %x", *(volatile unsigned long*)(VIC_INT_EN_L_V));
}
...
int __init musb_platform_init(struct musb *musb)
{
   struct otg_transceiver *xceiv;
	...
	/*reset usb-phy,modified by jerry-liang*/
	...
	printk("*********************\n");
	printk("before setup_timer \n");
    setup_timer(&musb_idle_timer, musb_do_idle, (unsigned long) musb);
	printk("*********************\n");
	printk("after setup_timer \n");
