...
//#define HLINA_START_ADDRESS 0x4C000000
#define HLINA_START_ADDRESS 0x4D000000//start from 208MB
#endif
...
/*********************************************************************************************************************/
                       /****by mzx****/
  memalloc_class = class_create(THIS_MODULE, "memalloc");/* create vpu class under /sysfs */
  if(IS_ERR(memalloc_class)) 
     {
 //         printk("Err: failed in creating memalloc class.\n");
          return -1; 
      } 
...
/**************************************************************************************/
  device_destroy(memalloc_class, MKDEV(memalloc_major, 0));//need to be tested under KO mode
...
static int AllocMemory(unsigned *busaddr, unsigned int size, struct file *filp)
{
    ...
    *busaddr = 0;
    ...
    {
        printk("memalloc: id:%d, size:%d, reserved: %d, addr=%p\n", i, size, hlina_chunks[i].size, *busaddr  );
    }
