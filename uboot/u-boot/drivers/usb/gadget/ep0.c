...
extern u8 qualifier_desc[];
extern u8 config_full[];
extern u8 config_full_total[];
	...
	case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
		{
			 ...
			 */
            copy_config (urb, qualifier_desc, 10, max);
			break;
		}
    case USB_DESCRIPTOR_TYPE_OTHER_SPEED_CONFIGURATION:
		{
			...
			if(max == 9)
			{
				copy_config (urb, config_full, 9, max);
			}
			else if(max == 32)
			{
				copy_config (urb, config_full_total, 32, max);
			}
		}
        break;
	default:
