...
#if 1    
static inline void PrintEp0Pkt(u8 *pt)
{
    int i;
   DbgPrintf("[RCV:");
    for(i=0;i<18;i++)
...
static void musb_peri_ep0_zero_data_request(int err)
{
	...
	if (err) {
		...
		case USB_REQ_SET_CONFIGURATION:
			...
			//printf("USB_REQ_SET_CONFIGURATION done!\n");
		   isUsbdSetConfiguration=1; //fgs
			break;
		}
...
static void musb_peri_rx_ep(unsigned int ep)
{
	...
	/*fgs add +*/
		if(ep == 1)
		{
			endpoint = GET_ENDPOINT(udc_device, 1);
		}
		else if(ep == 2)
		{
			endpoint = GET_ENDPOINT(udc_device, 2);
		}
		else
		{
			endpoint = NULL;
	...
	/*fgs add -*/
		if (endpoint && endpoint->rcv_urb) 
		{
			...
			{
				urb->actual_length = 0;
				remaining_space = urb->buffer_length;
			}
			if (remaining_space) {
				int urb_bad = 0; /* urb is good */
...
static void just_for_ep2(unsigned int ep)
{
	u16 peri_rxcount = readw(&musbr->ep[ep].epN.rxcount);
	...
	//	endpoint = GET_ENDPOINT(udc_device, 2);  
		if(downloadFileSize==0 && ep2_first == 0)//it is the first packit
		{
			read_fifo(ep, 8, (void *)downPt);
            //The format of data: <ADDR(DATA):4>+<SIZE(n+10):4>+<DATA:n>+<CS:2>
            downloadAddress=
			    	*((u8 *)(downPt+0))+
					(*((u8 *)(downPt+1))<<8)+
					(*((u8 *)(downPt+2))<<16)+
			...
			downloadFileSize=
			    	*((u8 *)(downPt+4))+
					...
					(*((u8 *)(downPt+6))<<16)+
					(*((u8 *)(downPt+7))<<24);
			serial_printf("File info:\n");
			serial_printf("  Target FileSize = 0x%x Bytes\n",downloadFileSize - 10);
			serial_printf("  Target Address  = %08x \n",downloadAddress);
			
			checkSum=0;

	    	downPt=(u8 *)downloadAddress;//A ha, change the downPt for new addr here

	    	read_fifo(ep, peri_rxcount - 8, (void *)downPt);
			//The first 8-bytes are deleted. and we copy the remaining datas to the target addr	    
  	    	downPt += (peri_rxcount - 8);  
            totalDmaCount = peri_rxcount - 8;
			ep2_first = 1;

            fifo_full_ok = 1; //received a packet data
			
            musb_peri_rx_ack(2);
		}
		else
		{  
            read_fifo(ep, peri_rxcount, (void *)downPt);    
            downPt+=peri_rxcount; 
            totalDmaCount += peri_rxcount;
			musb_peri_rx_ack(2);
		}
	} 
	else 
...
static void musb_peri_rx(u16 intr)
{
	...
	for (ep = 1; ep < 16; ep++) //deal with all endpoints
	{
		if ((1 << ep) & intr)
		{
			if(ep == 2)
